...
class BanAdmin(admin.ModelAdmin):
    fields = (
        ...
        "expires",
        "reason",
        "append_standard_reason",
        "reaction",
        "reaction_includes_ipbans",
    )
    ...
    def save_model(self, request, obj, form, change):
        ...
        if webhook_url := getattr(settings, "ADMIN_REPORT_WEBHOOK", ""):
            send_save_model_notification(
                webhook_url, request, change, ":hammer:", "Ban", obj.id
            )
    ...
    def delete_model(self, request, obj):
        ...
        if webhook_url := getattr(settings, "ADMIN_REPORT_WEBHOOK", ""):
            send_delete_model_notification(
                webhook_url, request, ":hammer:", "Ban", obj_id
            )
    ...
    def delete_queryset(self, request, queryset):
        ...
        if webhook_url := getattr(settings, "ADMIN_REPORT_WEBHOOK", ""):
            send_delete_queryset_notification(
                webhook_url, request, ":hammer:", "Ban", obj_ids
            )
