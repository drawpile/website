document.addEventListener("DOMContentLoaded", () => {
	const makeModal = (activationButtonId, makeContent, onOkClicked) => {
		...
		activationButton.onclick = () => {
			...
			modal.innerHTML += '<button class="modal-close is-large cancel" aria-label="close"></button>';

			const closeModal = () => modal.remove();
			modal.querySelectorAll(".cancel").forEach(e => e.onclick = closeModal);
	...
	makeModal('leave-group-button', (modal, button) => {
		const adminText = button.dataset.admin === 'true' ?
	...
	makeModal('abuse-report-button', (modal) => {
		modal.innerHTML = `
			<div class="modal-background cancel"></div>
