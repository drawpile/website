...
(function () {
  ...
  const listingTable = (setSortFunc, enableNsfm) =>
    el(
      "div",
      { class: "root" },
      el(
          ...
          el(
            ...
            el(
              ...
              el(
                ...
                {
                  class: "sortable",
                  "data-column": "activedrawingusers",
                  click: setSortFunc,
                },
                "Active"
              ),
  ...
  const versionFlair = (session) => {
    ...
    return el(
      "span",
      { class: "flair " + version.className, title: version.title },
      version.text
    );
  };
  ...
  const buildInviteLink = (session) => {
    const host =
      session.port === 27750 ? session.host : `${session.host}:${session.port}`;
  ...
  class SessionList extends HTMLElement {
    ...
    connectFilter(key, defaultValue) {
      ...
      if (filter) {
        ...
        const initialValue = getFilterFromLocalStorage(
          localStorageKey,
          defaultValue
        );
    ...
    setError(message) {
      ...
      tbody.innerHTML = "";
      tbody.append(
        el("tr", {}, el("td", { class: "error", colspan: 6 }, message))
      );
    }
    ...
    updateTable() {
      ...
      if (this._sessions.length === 0) {
        ...
        const sessions = [...this._sessions]
          ...
          .filter((s) => {
            return (
              (this._filters.password || !s.password) &&
