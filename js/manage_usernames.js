...
const max_users = parseInt(
  document.querySelector("meta[name='max-users']").content
);
...
async function sendData(url, method, data) {
  ...
  if (response.status === 400) {
    ...
    if (entries.length === 1) throw new Error(entries[0][1].join(", "));
    throw new Error(err);
  }
  if (!response.ok) {
      ...
      <div class="media-content">
        <div class="content">
          <p>
            ...
            ${isMod &&
            html` <button
              class="button is-text ${loading === "is_mod" ? "is-loading" : ""}"
              onclick=${update("is_mod", !user.is_mod)}
            >
              <span class="icon"
                ><i class="fas fa-${user.is_mod ? "check-" : ""}circle"></i
              ></span>
            ...
            user.is_mod &&
            html` <button
              class="button is-text ${loading === "is_ghost"
                ? "is-loading"
                : ""}"
              onclick=${update("is_ghost", !user.is_ghost)}
            >
              <span class="icon"
                ><i class="fas fa-${user.is_ghost ? "check-" : ""}circle"></i
              ></span>
          ...
          <p>
            ...
            html`
              <button
                class="button is-text ${loading === "is_primary"
                  ? "is-loading"
                  : ""}"
                onclick=${update("is_primary", true)}
              >
                <span class="icon"><i class="fas fa-arrow-up"></i></span>
                <span>Make primary</span>
              </button>
              <button
                class="button is-text ${loading === "remove"
                  ? "is-loading"
                  : ""}"
                onclick=${remove}
              >
            ...
            />
            <button
              class="button is-text  ${loading === "avatar"
                ? "is-loading"
                : ""}"
...
function App({ permissions }) {
  ...
  return html`
    ${error !== null &&
    html`<p class="notification is-danger">${error.toString()}</p>`}
