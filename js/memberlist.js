...
const memberlist_url = document.querySelector(
  "meta[name='memberlist-url']"
).content;
  ...
  if (status === "pending") {
    ...
    return html`
      <div class="buttons has-addons">
        <button
          class="button is-small ${status === "member"
        ...
        <button
          class="button is-small ${status === "mod"
            ? "is-selected is-success"
            : ""}"
          onclick=${update("mod")}
        >
        ...
        <button
          class="button is-small ${status === "admin"
            ? "is-selected is-success"
            : ""}"
...
function MemberRow({ user, refresh, setError }) {
  ...
  return html` <tr>
    ...
    <td>
      <button class="button is-small is-danger" onclick=${deleteUser}>
...
function Memberlist({ users, refresh, setError }) {
  return html`<table class="table">
    ${users.map(
      (u) =>
        html`<${MemberRow} user=${u} refresh=${refresh} setError=${setError} />`
    )}
  </table>`;
}
